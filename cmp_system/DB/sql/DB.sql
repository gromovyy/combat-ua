...
------------Создание процедуры "db_proc_show_var"------------

DELIMITER $$
DROP PROCEDURE IF EXISTS `db_proc_show_var` $$
CREATE PROCEDURE `db_proc_show_var`(IN `varname` VARCHAR(64) CHARSET utf8)
    NO SQL
    COMMENT 'Выводит значение переменной'
BEGIN
  SET @s = CONCAT('SELECT @',varname);
    PREPARE stmt FROM @s;
    EXECUTE stmt;
...
------------Создание процедуры "db_proc_table_fix"------------

DELIMITER $$
DROP PROCEDURE IF EXISTS `db_proc_table_fix` $$
CREATE PROCEDURE `db_proc_table_fix`(IN `v_table` VARCHAR(50))
    NO SQL
BEGIN

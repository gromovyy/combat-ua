<?php 
	...
	$view_update_link = "Attachment/ResultUniversal/{$project["id_project"]}/{$attachment["id_object"]}";
?>
<div>
	<div class="attachment_file_block" id="a<?php echo $attachment["id_attachment"]?>">
		...
		<?php if ($this->is_update($attachment["id_owner"], $attachment["id_attachment"])) { ?>					
			<div class="btnAttachment" onclick="$('#project_id').val('<?php echo $project["id_project"]; ?>'); $('#project_image_id').val('<?php echo $project["id_photo"]; ?>');  $('#project_attachment_id').val('<?php echo $attachment["id_attachment"]; ?>'); $('#project_upload_dialog').fadeIn();">Прикріпити файл</div>
		<?php } ?>
    </div>
</div>
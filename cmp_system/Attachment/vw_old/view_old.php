...
<div>
	<div class="attachment_file_block" id="a<?php echo $attachment["id_attachment"]?>">
		...
		<?php if ($this->is_update($attachment["id_owner"], $attachment["id_attachment"])) { ?>					
			<div class="btnEdit" onclick="$('#attachment_upload_id').val(<?php echo $attachment["id_attachment"];?>);$('#attachment-title').val('<?php echo $attachment["title"];?>');$('#attachment_upload_dialog').fadeIn();"></div>
		<?php } ?>
    </div>
</div>